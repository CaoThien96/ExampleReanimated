...
export default class BottomSheetBehavior extends React.Component {
    ...
    velocity = new Value(0)
    panMasterState = new Value(GestureState.END)
    ...
    constructor(props) {
        ...
        for (let i = 1; i < snapPoints.length; i++) {
            const tuple = [
                add(snapPoints[i - 1], screenHeight / 2),
                sub(snapPoints[i], screenHeight / 2),
            ]
            console.log('DCM snapPoints[i - 1], screenHeight / 2', snapPoints[i - 1] + screenHeight / 2)
            console.log('DCM snapPoints[i], screenHeight / 2', snapPoints[i] - screenHeight / 2)
        ...
        const currentSnapPoint = (i = 0) =>
            i + 1 === snapPoints.length
                ...
                : cond(
                    positive,
                    cond(
                        greaterThan(destinationPoint, screenHeight / 2),
                        snapPoints[i],
                        snapPoints[i + 1] //0
                    ),
                    snapPoints[i + 1]
